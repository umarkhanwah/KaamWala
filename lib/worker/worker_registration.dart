...
class WorkerRegistrationPage extends StatefulWidget {
  ...
  @override
  State<WorkerRegistrationPage> createState() =>
      _WorkerRegistrationPageState();
}
...
class _WorkerRegistrationPageState extends State<WorkerRegistrationPage>
  ...
  // ──────────────────────────────────────────────
  final List<String> quickTips = const [
    "✅ Keep your profile accurate for faster approval.",
    "📞 Stay reachable on your registered phone number.",
    "🛠 Verified workers get more job requests.",
    "🔔 Approval notification is sent automatically.",
  ];
  int _currentTipIndex = 0;
  ...
  void initState() {
    ...
    Future.doWhile(() async {
      ...
      setState(() {
        _currentTipIndex =
            (_currentTipIndex + 1) % quickTips.length;
      });
      return true;
    });
  }
  ...
  Widget _verificationBanner(String status) {
    ...
    if (approved) {
      ...
      desc =
          "Your profile is approved. You can now receive job requests.";
    } else if (rejected) {
      ...
      start = Colors.orange.shade400;
      end = Colors.deepOrange;
      icon = Icons.hourglass_top_rounded;
      title = "Verification Pending";
      desc =
          "Admin approval is required. You will be notified soon.";
    }
        ...
        children: [
          ...
          Expanded(
            child: AnimatedSwitcher(
              ...
              child: Text(
                quickTips[_currentTipIndex],
                key: ValueKey(quickTips[_currentTipIndex]),
                style: const TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.w600,
                ),
    ...
    return ScaleTransition(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(16),
          gradient: const LinearGradient(
            colors: [Color(0xFFEBF3FF), Colors.white],
          ),
          boxShadow: [
            BoxShadow(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: TweenAnimationBuilder<double>(
                tween:
                    Tween(begin: 0, end: value.toDouble()),
                duration:
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                  ...
                  builder: (context, snapshot) {
                    ...
                    final status =
                        snapshot.data!.data()?["status"] ??
                            "Pending";
                    return _verificationBanner(status);
                  },
                ),
                ...
                _sectionHeader(
                    "Overview", Icons.space_dashboard),
                SingleChildScrollView(
                  scrollDirection: Axis.horizontal,
                  child: Row(
                    children: [
                      _animatedStat("Jobs Done", 120,
                          Icons.task_alt_outlined),
                      const SizedBox(width: 12),
                      _animatedStat("Requests", 42,
                          Icons.inbox_outlined),
                      const SizedBox(width: 12),
                      _animatedStat("Rating", 5,
                          Icons.star_outline),
                    ],
                  ),
                ),
